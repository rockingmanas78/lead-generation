...
class LLMParser:
    ...
    async def extract_missing_fields(self, content: str, missing_fields: List[str]) -> Dict:
        ...
        requested = [f"- {field_map[field_key]}" for field_key in missing_fields if field_key in field_map]
        if not requested:
            return {}

        requested_fields = '\n'.join(requested)
        json_fields = ', '.join([
            f'"{field_key}": []' if field_key in ["emails", "phones", "addresses"] else f'"{field_key}": ""'
            for field_key in missing_fields if field_key in field_map
        ])
